...
/**
 * Check whether Vercel Blob storage is configured.
 */
...
export async function storeUpload(
  ...
  if (file.size > MAX_BYTES) {
    throw new Error(
      'That file is larger than 5 MB. Please compress it and try again.',
    );
  }
  ...
  if (!allowed.includes(file.type)) {
    throw new Error(
      `Files of type "${file.type || 'unknown'}" are not allowed. Use an image, PDF or Word document.`,
    );
  }
  ...
  const safeFolder =
    folder.replace(/[^a-z0-9_-]/gi, '') || 'media';
...
/**
 * List uploaded media from Vercel Blob.
 */
...
export function humanSize(bytes: number): string {
  ...
  return `${
    i === 0 ? Math.round(value) : value.toFixed(1)
  } ${units[i]}`;
}
