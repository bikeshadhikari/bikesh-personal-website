...
export const RESOURCES: Record<string, ResourceDef> = {
  ...
  notices: {
    table: 'notices', label: 'Pop-up notices', singular: 'Notice',
    icon: 'sparkle', group: 'Content', order: 'sort_order ASC, id DESC',
    search: ['title', 'body'], toggle: 'enabled',
    list: [
      { key: 'title', label: 'Title', primary: true },
      { key: 'image', label: 'Image', type: 'thumb' },
    ...
    fields: {
      title: { type: 'text', label: 'Title', full: true,
               hint: 'The heading on the notice. Leave empty to show the picture on its own.' },
      image: { type: 'image', label: 'Image', folder: 'notices', full: true,
               hint: 'Any size or shape. The whole picture is shown, never cropped.' },
      body: { type: 'textarea', label: 'Notice', rows: 4, full: true,
              hint: 'The message under the picture. Blank lines start a new paragraph.' },
      link_file: { type: 'file', label: 'Attach a PDF', folder: 'notices',
                   hint: 'Upload a document and the button opens it. Used in place of the link below.' },
      link_url: { type: 'url', label: 'Or link somewhere',
                  hint: 'Another website, or a page on this one such as /blog. Ignored when a PDF is attached.' },
      link_label: { type: 'text', label: 'Button text', default: 'Read more',
                    hint: 'Only used when a link is set.' },
      starts_at: { type: 'datetime', label: 'Show from',
